...
<div class="p-6">
    <div class="mb-6 flex justify-between items-center">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">Add New Child</h1>
            <p class="text-gray-600">
                @if(isset($redirectToSale) && $redirectToSale)
                    Register a child to create a sale
                @else
                    Register a new child in the system
                @endif
            </p>
        </div>
        <a href="{{ isset($redirectToSale) && $redirectToSale ? route('cashier.children.index', ['select_for_sale' => 'true']) : route('cashier.children.index') }}"
    ...
    <div class="bg-white rounded-lg shadow-md">
        ...
        <form action="{{ route('cashier.children.store') }}" method="POST" class="p-6">
            @csrf
            
            @if(isset($redirectToSale) && $redirectToSale)
                <input type="hidden" name="redirect_to_sale" value="true">
            @endif
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    <label for="guardian_name" class="block text-gray-700 text-sm font-medium mb-2">Guardian
                ...
                <div>
                    <label for="guardian_phone" class="block text-gray-700 text-sm font-medium mb-2">Guardian
