    ...
    <form action="{{ route('admin.addons.update', $addon) }}" method="POST">
        ...
        <div class="mb-4">
            <label class="flex items-center">
                <input type="checkbox" name="active" value="1" class="form-checkbox h-5 w-5 text-blue-600" {{ old('active', $addon->active) ? 'checked' : '' }}>
                <span class="ml-2 text-gray-700 text-sm font-bold">Active Add-on</span>
            </label>
            <p class="text-gray-500 text-xs mt-1">Uncheck to discontinue this add-on</p>
        </div>
